            ...
            try:
                ...
                for attempt in range(num_retries):
                    try:
                        ...
                        # if there are more retries left, try to recover the page
...
def _click_got_it_if_present(page: Page) -> None:
    ...
    """
    got_it_buttons = page.get_by_role("button", name=re.compile(r"^\s*got\s+it\s*$", re.IGNORECASE))

    try:
        total = got_it_buttons.count()
    except Exception:
        return

    for index in range(total):
        try:
            button = got_it_buttons.nth(index)
...
def _go_to_upload(page: Page) -> None:
    ...
    retry once before giving up.  A timeout during the ``#root`` wait is the
    most common symptom.
    """
    logger.debug(green("Navigating to upload page"))

    # try twice: first attempt, then a retry after a short delay
    def _wait_for_ready() -> None:
        """Wait for the upload form to be reachable.

        TikTok sometimes renders ``#root`` but keeps it hidden behind an
        overlay; the video-input element is a more reliable indicator that the
        page is actionable.  This helper tries both selectors before bailing out.
        """
        try:
            page.wait_for_selector("#root", timeout=config.explicit_wait * 1000)
        except Exception:
            # fallback: wait for the file-input control (may already be visible)
    ...
    for attempt in range(2):
        if page.url != config.paths.upload:
            ...
            # refresh the current upload page (should be same URL)
            page.reload()
        ...
        try:
            ...
            if attempt == 0:
                ...
                logger.info(green("Retrying navigation to upload page"))
                time.sleep(1)
                continue
            # second attempt also failed — propagate the error
            raise
