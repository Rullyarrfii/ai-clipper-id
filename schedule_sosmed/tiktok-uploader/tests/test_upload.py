...
def test_set_description_clears_existing_and_types() -> None:
    """
    Ensure that _set_description first wipes whatever text is already in the
    editable div (e.g. the auto-filled filename) before entering the supplied
    description words.
    """
    from tiktok_uploader.upload import _set_description

    mock_page = MagicMock()
    mock_locator = MagicMock()
    mock_page.locator.return_value = mock_locator

    # simulate existing text when we query the element
    def fake_eval(js):
        # return some text when checking innerText
        if "innerText" in js:
            return "video.mp4"
        return None
    ...
    _set_description(mock_page, "hello world")

    # clear should have been invoked at least once with control commands
    assert mock_locator.press.call_count >= 2
    assert mock_locator.press.call_args_list[0][0][0] in ("Control+A",)
    assert mock_page.evaluate.called
    # typing of description should occur (press_sequentially used)
    assert mock_locator.press_sequentially.call_count >= 2


def test_set_description_empty_string_clears_only() -> None:
    """
    Passing an empty string should still clear the field but not attempt to
    type anything afterwards.
    """
    from tiktok_uploader.upload import _set_description
    ...
    mock_locator = MagicMock()
    mock_page.locator.return_value = mock_locator
    mock_page.evaluate.return_value = "some text"

    _set_description(mock_page, "")

    # field should be cleared and no sequential typing should occur
    assert mock_locator.press.call_count >= 2
    assert mock_locator.press_sequentially.call_count == 0
    assert mock_page.evaluate.called
